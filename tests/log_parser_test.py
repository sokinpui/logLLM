...
project_root = os.path.abspath(os.path.join(script_dir, '..')) # Adjust if structure differs
...
try:
    ...
    from logllm.agents.log_parser_agent import GrokPatternListSchema # Import the schema
...
PROMPT_KEY = "logllm.agents.log_parser_agent.generate_grok_patterns" # Key for the prompt
...
try:
    ...
    prompt_template = pm.get_prompt("logllm.agents.log_parser_agent.LogParserAgent.generate_parsing_rule", sample_logs=formatted_samples)
    ...
    # print("------------------------------------\n")
     ...
     logger.error(f"PromptsManager runtime error (maybe called outside instance method?): {e}")
...
# --- 3. Call LLM ---
logger.info(f"Sending prompt to LLM ({model.__class__.__name__}) for Grok pattern generation...")
print(f"Querying LLM ({model.__class__.__name__})... This may take a moment.")
try:
    response: GrokPatternListSchema = model.generate(prompt_template, schema=GrokPatternListSchema)
...
if response and response.grok_patterns:
    ...
    logger.error("LLM response was empty or not in the expected GrokPatternListSchema format.")
