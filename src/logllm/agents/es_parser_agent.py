...
class SingleGroupParseGraphState(TypedDict):
    ...
    final_parsing_results_summary: Optional[
        Dict[str, int]
    ]  # e.g., {"processed": N, "successful": M, "failed": O, "parse_errors": P, "index_errors": Q}
...
def _parallel_group_worker_new(
    single_group_config: Dict[str, Any], prompts_json_path: str
    ...
    try:
        ...
        if db_worker.instance is None:
            raise ConnectionError("Worker ES connection failed")
        ...
        sg_agent_worker = SingleGroupParserAgent(
            model=model_worker, db=db_worker, prompts_manager=prompts_manager_worker
        )
        ...
        worker_logger.info(
            f"[Worker-{os.getpid()}] Finished Group: {group_name}, Status: {final_state.get('final_parsing_status', 'unknown')}"
        )
        return group_name, final_state

    except Exception as e:
        worker_logger.error(
            f"[Worker-{os.getpid()}] CRITICAL Error in group '{group_name}': {e}",
            exc_info=True,
        )
        ...
        failed_state: SingleGroupParseGraphState = {
            ...
            "failed_index": cfg.get_unparsed_log_storage_index(
                group_name
            ),  # Best guess
            "field_to_parse": single_group_config.get("field_to_parse", "content"),
            "fields_to_copy": single_group_config.get("fields_to_copy"),
            "sample_size_generation": single_group_config.get(
                "sample_size_generation", 20
            ),
            "sample_size_validation": single_group_config.get(
                "sample_size_validation", 10
            ),
            "validation_threshold": single_group_config.get(
                "validation_threshold", 0.5
            ),
            "batch_size": single_group_config.get("batch_size", 1000),
            "max_regeneration_attempts": single_group_config.get(
                "max_regeneration_attempts", 3
            ),
            "keep_unparsed_index": single_group_config.get(
                "keep_unparsed_index", False
            ),  # Include this
            "provided_grok_pattern": single_group_config.get(
                "provided_grok_pattern"
            ),  # Include this
            "current_attempt": 0,
...
class ScrollGrokParserAgent:
    ...
    _current_success_batch_data: List[
        Tuple[str, Dict[str, Any]]
    ] = []  # (source_id, parsed_doc) for target index
    _current_failed_batch_data: List[
        Tuple[str, Dict[str, Any], str]
    ] = []  # (source_id, original_source_doc, reason) for failed index
    ...
    def _initialize_grok(self, pattern: str) -> bool:
        try:
            ...
            self._logger.error(
                f"Invalid Grok pattern syntax: {pattern} - Error: {e}", exc_info=True
            )
    ...
    def _process_single_hit(
        ...
        if not isinstance(original_content, str):
            self._logger.warning(
                f"Field '{field_to_parse}' not found/string in doc ID {source_id}. Storing as failed."
            )
            self._current_failed_batch_data.append(
                (source_id, source_doc, "missing_parse_field")
            )
        ...
        if is_fallback:
            self._current_failed_batch_data.append(
                (source_id, source_doc, "fallback_used")
            )
        ...
        if self._grok_instance is None:
            self._logger.error(
                "Grok instance not initialized during hit processing. This shouldn't happen."
            )
            # Store as failed if grok isn't ready (unexpected state)
            self._current_failed_batch_data.append(
                (source_id, source_doc, "internal_grok_error")
            )
            return "parse_failed"
        ...
        if parsed_fields:
            ...
            self._current_failed_batch_data.append(
                (source_id, source_doc, "grok_mismatch")
            )
            return "parse_failed"

    def _process_batch(
        self, hits: List[Dict[str, Any]], state: ScrollGrokParserState
    ) -> bool:
        ...
        self._parse_error_count += (
            batch_parse_failures  # Update agent's total parse error count
        )
    ...
    def _flush_success_batch(self, target_index: str):
        ...
        if not self._current_success_batch_data:
            return

        self._logger.info(
            f"Flushing SUCCESS batch of {len(self._current_success_batch_data)} documents to target index '{target_index}'..."
        )
        bulk_actions = []
        ...
        if errors:
            self._logger.warning(
                f"{len(errors)} errors occurred during target index bulk update/upsert."
            )
    ...
    def _flush_failed_batch(self, failed_index: str):
        ...
        if not self._current_failed_batch_data:
            return

        self._logger.info(
            f"Flushing FAILED/FALLBACK batch of {len(self._current_failed_batch_data)} documents to failed index '{failed_index}'..."
        )
        bulk_actions = []
        for source_id, original_source, reason in self._current_failed_batch_data:
            failed_doc = {
                ...
                "@timestamp": original_source.get(
                    "@timestamp", datetime.now().isoformat()
                ),  # Preserve or add timestamp
            }
            bulk_actions.append(
                {
        ...
        success_count, errors = self._db.bulk_operation(actions=bulk_actions)
        self._failed_indexed_count += (
            success_count  # Count docs successfully written to failed index
        )
        self._index_error_count += len(errors)
        if errors:
            self._logger.warning(
                f"{len(errors)} errors occurred during failed index bulk insert."
            )
    ...
    def run(self, state: ScrollGrokParserState) -> ScrollGrokParserState:
        ...
        log_prefix = (
            f"Fallback Run ({failed_index})"
            if is_fallback
            else f"Parse Run ({target_index})"
        )
        self._logger.info(
            f"Starting ScrollGrokParserAgent {log_prefix}. Source: '{source_index}', Batch: {self._batch_size_this_run}"
        )
        ...
        if not is_fallback:
            if not self._initialize_grok(pattern):
                ...
                self._logger.error(
                    f"Grok pattern invalid: {pattern}. Cannot proceed with normal parsing."
                )
                ...
                return {
                    **state,
                    "status": "failed",
                    "parse_error_count": -1,
                }  # Indicate Grok init failure

        # Prepare query and fields
        source_query = state.get("source_query") or {"query": {"match_all": {}}}
        fields_needed = set([state["field_to_parse"]])
        if state.get("fields_to_copy"):
            fields_needed.update(state["fields_to_copy"])
        if "@timestamp" not in fields_needed:
            fields_needed.add("@timestamp")  # Ensure timestamp is fetched
        ...
        try:
            processed_count, _ = self._db.scroll_and_process_batches(
                ...
                process_batch_func=lambda hits: self._process_batch(
                    hits, state
                ),  # Pass state to processor
            ...
            final_status = "failed"
            self._logger.error(
                f"Run failed during scroll/processing: {e}", exc_info=True
            )
            # Attempt to flush remaining data even on error
            try:
                self._flush_success_batch(target_index)
            except Exception as flush_err:
                self._logger.error(f"Error flushing final success batch: {flush_err}")
            try:
                self._flush_failed_batch(failed_index)
            except Exception as flush_err:
                self._logger.error(f"Error flushing final failed batch: {flush_err}")
        ...
        self._logger.info(
            f"{log_prefix} Summary: Processed={result_state['processed_count']}, "
...
class SingleGroupParserAgent:
    ...
    def _start_node(
        self, state: SingleGroupParseGraphState
    ) -> SingleGroupParseGraphState | Dict[str, Any]:
        ...
        provided_pattern = state.get(
            "provided_grok_pattern"
        )  # Retrieve provided pattern from the *initial* state
        ...
        try:
            ...
            self._logger.info(
                f"[{group_name}] Indices: Source='{source_index}', Target='{target_index}', Failed='{failed_index}'"
            )
            ...
            if not keep_unparsed_index:
                try:
                    self._logger.info(
                        f"[{group_name}] Deleting existing unparsed index: {failed_index}"
                    )
                    delete_resp = self._db.instance.indices.delete(
                        index=failed_index, ignore=[400, 404]
                    )
                    self._logger.info(
                        f"[{group_name}] Delete response for {failed_index}: {delete_resp}"
                    )
                except Exception as del_e:
                    self._logger.warning(
                        f"[{group_name}] Could not delete unparsed index {failed_index} (might not exist): {del_e}"
                    )
            else:
                self._logger.info(
                    f"[{group_name}] Skipping deletion of unparsed index: {failed_index} (--keep-unparsed specified)"
                )
            # ---------------------------------------------
            ...
            if not provided_pattern:
                self._logger.info(
                    f"[{group_name}] No pattern provided, fetching samples..."
                )
                # Fetch samples (generation)
                self._logger.info(
                    f"[{group_name}] Fetching {state['sample_size_generation']} samples for generation from '{source_index}'..."
                )
                ...
                if not samples_gen:
                    self._logger.warning(
                        f"[{group_name}] No samples found for generation."
                    )

                # Fetch samples (validation)
                self._logger.info(
                    f"[{group_name}] Fetching {state['sample_size_validation']} samples for validation from '{source_index}'..."
                )
                ...
                self._logger.info(
                    f"[{group_name}] Using provided pattern: {provided_pattern}. Skipping sample fetching."
                )
    ...
    def _generate_grok_node(
        ...
        self._logger.info(
            f"[{group_name}] Attempt {attempt}: Generating Grok pattern..."
        )

        if not samples:
            msg = (
                f"[{group_name}] No samples available for generation attempt {attempt}."
            )
        ...
        try:
            ...
            if (
                response
                and isinstance(response, GrokPatternSchema)
                and response.grok_pattern
            ):
                ...
                if pattern == last_failed:
                    ...
                    self._logger.info(
                        f"[{group_name}] LLM generated Grok pattern (Attempt {attempt}): {pattern}"
                    )
    ...
    def _validate_pattern_node(
        ...
        try:
            ...
            if total_validated == 0:
                self._logger.warning(f"[{group_name}] No non-empty validation lines.")
                validation_passed = False
            else:
                success_rate = parsed_count / total_validated
                self._logger.info(
                    f"[{group_name}] Validation: Parsed {parsed_count}/{total_validated}. Rate: {success_rate:.2f}"
                )
                if success_rate >= threshold:
                    self._logger.info(
                        f"[{group_name}] Validation PASSED (>= {threshold:.2f})."
                    )
                    validation_passed = True
                else:
                    self._logger.warning(
                        f"[{group_name}] Validation FAILED (< {threshold:.2f})."
                    )
                    validation_passed = False
        except ValueError as e:
            msg = (
                f"[{group_name}] Invalid Grok syntax during validation: {pattern} - {e}"
            )
            self._logger.error(msg)
            errors.append(msg)
            validation_passed = False
    ...
    def _parse_all_node(
        ...
        failed_index = state["failed_index"]
        self._logger.info(
            f"[{group_name}] Validation passed. Parsing all documents -> Target: {target_index}, Failed/Skipped -> {failed_index}"
        )

        if not pattern:  # Safety check
            msg = (
                f"[{group_name}] Critical error: Pattern is missing in _parse_all_node."
            )
            self._logger.error(msg)
            return {
        ...
        try:
            ...
            if final_sub_state.get("status") == "completed":
                ...
                if (
                    final_sub_state.get("parse_error_count", 0) > 0
                    or final_sub_state.get("index_error_count", 0) > 0
                    or final_sub_state.get("failed_indexed_count", 0) > 0
                ):  # Count docs sent to failed index as 'errors' in this context
                    final_status = "success_with_errors"
                    self._logger.warning(
                        f"[{group_name}] Parse all completed with some errors/failures."
                    )
                else:
                    final_status = "success"
                    self._logger.info(
                        f"[{group_name}] Parse all completed successfully."
                    )
            else:
                self._logger.error(
                    f"[{group_name}] Parse all failed. Sub-agent status: {final_sub_state.get('status')}"
                )
            ...
            results_summary = {
                ...
                "parse_errors": final_sub_state.get(
                    "parse_error_count", 0
                ),  # Grok mismatches within this run
                "index_errors": final_sub_state.get(
                    "index_error_count", 0
                ),  # Bulk indexing errors
            }
            return {
                "final_parsing_status": final_status,
                "final_parsing_results_summary": results_summary,
            }
        except Exception as e:
    ...
    def _fallback_node(
        ...
        failed_index = state["failed_index"]
        self._logger.warning(
            f"[{group_name}] Executing FALLBACK. ALL documents will be stored in: {failed_index}"
        )
        ...
        try:
            ...
            final_sub_state = self._scroll_parser_agent.run(scroll_config)
            final_status = (
                "failed_fallback"  # Assume failure unless explicitly completed
            )
            if final_sub_state.get("status") == "completed":
                ...
                self._logger.error(
                    f"[{group_name}] Fallback processing FAILED. Sub-agent status: {final_sub_state.get('status')}"
                )

            # Store summary - note: successfully_indexed_count should be 0 in fallback
            results_summary = {
                "processed": final_sub_state.get("processed_count", 0),
                "successful": final_sub_state.get(
                    "successfully_indexed_count", 0
                ),  # Should be 0
                "failed": final_sub_state.get(
                    "failed_indexed_count", 0
                ),  # All processed docs end up here
                "parse_errors": final_sub_state.get(
                    "parse_error_count", 0
                ),  # Should be equal to processed
    ...
    def _prepare_for_retry_node(
        ...
        self._logger.info(
            f"[{group_name}] Preparing for retry (Attempt {current_attempt + 1}). Storing failed pattern: {failed_pattern}"
        )
        return {
    ...
    def _store_results_node(self, state: SingleGroupParseGraphState) -> Dict[str, Any]:
        ...
        self._logger.info(
            f"[{group_name}] Storing parsing results to history index. Status: {status}"
        )
        ...
        if status == "success_fallback":
            ...
            "failed" in status and not pattern_used
        ):  # If failed early, pattern might be None
            pattern_used = (
                state.get("last_failed_pattern")
                or "N/A (Failed before pattern selection)"
            )
        ...
        result_doc = {
            ...
            if summary
            else 0,  # Docs sent to failed index
            "parse_error_count": summary.get("parse_errors", 0)
            if summary
            else 0,  # Grok mismatches
            "index_error_count": summary.get("index_errors", 0)
            if summary
            else 0,  # Bulk errors
            "agent_error_count": len(
                state.get("error_messages", [])
            ),  # Count agent-level errors
        }

        try:
            self._db.insert(data=result_doc, index=cfg.INDEX_GROK_RESULTS_HISTORY)
            self._logger.info(
                f"[{group_name}] Successfully stored results in '{cfg.INDEX_GROK_RESULTS_HISTORY}'."
            )
        except Exception as e:
    ...
    def _decide_pattern_source(self, state: SingleGroupParseGraphState) -> str:
        ...
        if state.get("current_grok_pattern"):
            self._logger.debug(
                f"[{group_name}] Pattern provided or already set, moving to validation."
            )
            return "validate_pattern"  # Skip generation
        else:
            self._logger.debug(
                f"[{group_name}] No pattern provided, moving to generation."
            )
    ...
    def _decide_after_generate(self, state: SingleGroupParseGraphState) -> str:
        ...
        if state.get("error_messages") and any(
            "No samples found for validation" in msg for msg in state["error_messages"]
        ):
            self._logger.error(
                f"[{group_name}] Cannot validate (no validation samples), moving to fallback."
            )
            return "fallback"

        if state.get("current_grok_pattern"):
            self._logger.debug(
                f"[{group_name}] Pattern generated, moving to validation."
            )
            ...
            if current_attempt < max_attempts:
                self._logger.warning(
                    f"[{group_name}] Pattern generation failed (Attempt {current_attempt}/{max_attempts}), preparing for retry."
                )
                # Need to go through retry prep to increment attempt count
                return "prepare_for_retry"
            else:
                self._logger.error(
                    f"[{group_name}] Pattern generation failed after max attempts ({max_attempts}), moving to fallback."
                )
                return "fallback"
    ...
    def _decide_after_validation(self, state: SingleGroupParseGraphState) -> str:
        ...
        if state.get("validation_passed"):
            self._logger.debug(
                f"[{group_name}] Validation passed, moving to parse_all."
            )
            return "parse_all"
        else:
            ...
            if current_attempt < max_attempts:
                self._logger.warning(
                    f"[{group_name}] Validation failed (Attempt {current_attempt}/{max_attempts}), preparing for retry."
                )
                return "prepare_for_retry"
            else:
                self._logger.error(
                    f"[{group_name}] Validation failed after max attempts ({max_attempts}), moving to fallback."
                )
                return "fallback"
    ...
    def _build_graph(self) -> CompiledGraph:
        ...
        workflow.add_node(
            "store_results", self._store_results_node
        )  # Store results before END
        ...
        workflow.add_edge(
            "prepare_for_retry", "generate_grok"
        )  # Loop back to generate after prep
    ...
    def run(
        ...
        self._logger.info(
            f"[{group_name}] Initializing SingleGroupParserAgent graph run..."
        )
        ...
        graph_input: SingleGroupParseGraphState | Dict[str, Any] = {
            ...
            "keep_unparsed_index": initial_config.get(
                "keep_unparsed_index", False
            ),  # Pass this flag
            "provided_grok_pattern": initial_config.get(
                "provided_grok_pattern"
            ),  # Pass provided pattern here
        ...
        self._logger.info(
            f"[{group_name}] Finished SingleGroupParserAgent graph run. Final Status: {final_state.get('final_parsing_status')}"
        )
...
class AllGroupsParserAgent:
    def __init__(
        ...
        self._model = (
            model  # Keep for potential future use? Or remove if worker handles all
        )
        self._db = db
    ...
    def _get_all_groups(self, group_info_index: str) -> List[Dict[str, Any]]:
        ...
        self._logger.info(
            f"Fetching all group definitions from index '{group_info_index}'..."
        )
        try:
            ...
            if not groups_data:
                return []
            valid_groups = [
                {"group_name": hit["_source"]["group"]}
                for hit in groups_data
                if hit.get("_source", {}).get("group")
            ]
            self._logger.info(f"Fetched {len(valid_groups)} valid group definitions.")
            return valid_groups
        except Exception as e:
            self._logger.error(
                f"Failed to fetch groups from index '{group_info_index}': {e}",
                exc_info=True,
            )
            return []
    ...
    def run(
        ...
        provided_grok_pattern: Optional[
            str
        ] = None,  # <-- Accept pattern (only used for single group run)
    ) -> AllGroupsParserState:
        self._logger.info(
            f"Starting AllGroupsParserAgent run. Workers: {num_threads}, Batch: {batch_size}, GenSample: {sample_size}, ValSample: {validation_sample_size}, MaxAttempts: {max_regeneration_attempts}"
        )
        ...
        if not groups_to_process:
            result_state["status"] = "completed (no groups)"
            return result_state
        ...
        with Executor(max_workers=effective_num_workers) as executor:
            ...
            for future in as_completed(future_to_group):
                ...
                try:
                    ...
                    result_state["group_results"][group_name_future] = (
                        final_state_result
                    )
                    status_report = final_state_result.get(
                        "final_parsing_status", "unknown"
                    )
                    self._logger.info(
                        f"Received result for group: {group_name_future}. Status: {status_report}"
                    )
                except Exception as e:
                    self._logger.error(
                        f"Error retrieving result for group '{group_name_future}': {e}",
                        exc_info=True,
                    )
