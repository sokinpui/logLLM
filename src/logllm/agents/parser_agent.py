...
def _parse_file_worker(
    file_path: str, group_grok_pattern: Optional[str], show_progress: bool
    ...
    worker_logger = (
        Logger()
    )  # Each worker gets its own logger instance if needed, or use shared singleton
    ...
    try:
        ...
        if not output_path and group_grok_pattern:
            worker_logger.warning(
                f"Group Grok pattern failed for {os.path.basename(file_path)}. Trying file-specific pattern generation."
            )
            # Create new state *without* the pattern to trigger LLM generation
            fallback_state: SimpleGrokLogParserState = {
                "log_file_path": file_path,
                "grok_pattern": None,  # <-- Force generation
                "output_csv_path": "",
                "sample_logs": "",
                "parsed_lines": 0,
                "skipped_lines": 0,
            }
            fallback_result_state = worker_agent.run(
                fallback_state, show_progress=show_progress
            )
            output_path = fallback_result_state.get("output_csv_path") or None
            if output_path:
                worker_logger.info(
                    f"Fallback Grok pattern generation SUCCESSFUL for {os.path.basename(file_path)}"
                )
            else:
                worker_logger.warning(
                    f"Fallback Grok pattern generation FAILED for {os.path.basename(file_path)}"
                )
        ...
        worker_logger.error(
            f"Error in Grok worker for {os.path.basename(file_path)}: {e}",
            exc_info=True,
        )
...
class SimpleGrokLogParserAgent:
        ...
        if not os.path.exists(prompt_path):
            ...
            if not os.path.exists(prompt_path):
                raise FileNotFoundError(
                    f"Prompts file not found at primary or relative path: {prompt_path}"
                )
    ...
    def run(
        self, initial_state: SimpleGrokLogParserState, show_progress: bool = False
    ) -> SimpleGrokLogParserState:
        """Parse a single log file using Grok, conditionally generating the pattern via LLM."""
        if "log_file_path" not in initial_state or not os.path.isfile(
            initial_state["log_file_path"]
        ):
        ...
        grok_pattern = initial_state.get(
            "grok_pattern"
        )  # Get potential user-provided pattern
        ...
        if not grok_pattern:
            self._logger.info(
                f"Grok pattern not provided for {base_name}. Generating via LLM..."
            )
            grok_pattern = self._generate_grok_pattern(log_file_path)
            result["grok_pattern"] = (
                grok_pattern  # Store the generated pattern in the state
            )

            if not grok_pattern:
                self._logger.warning(
                    f"Failed to generate Grok pattern for {base_name}. Skipping parsing."
                )
                return result  # Cannot parse without a pattern

            self._logger.info(
                f"LLM generated Grok pattern for {base_name}: {grok_pattern}"
            )
        else:
            self._logger.info(
                f"Using provided Grok pattern for {base_name}: {grok_pattern}"
            )
        ...
        try:
            ...
            self._logger.error(
                f"Error during Grok parsing execution for {base_name}: {e}",
                exc_info=True,
            )
    ...
    def _generate_grok_pattern(self, log_file_path: str) -> Optional[str]:
        ...
        try:
            with open(log_file_path, "r", encoding="utf-8", errors="ignore") as f:
                ...
                if not lines:
                    self._logger.warning(
                        f"Log file is empty, cannot sample: {log_file_path}"
                    )
                    return None
            ...
            self._logger.error(
                f"Failed to read or sample log file {os.path.basename(log_file_path)}: {e}",
                exc_info=True,
            )
            return None
        ...
        if not sample_logs:
            self._logger.warning(
                f"No log lines sampled from {os.path.basename(log_file_path)}, cannot generate pattern."
            )
            return None
        ...
        try:
            ...
            if (
                ...
                if "%{" in pattern and "}" in pattern:
                    ...
                    self._logger.warning(
                        f"LLM response doesn't look like a valid Grok pattern: {pattern}"
                    )
                    return None
            else:
                self._logger.warning(
                    f"LLM did not return a valid GrokPatternSchema object. Response: {response}"
                )
                return None

        except (
            ValueError
        ) as ve:  # Catch potential Pydantic validation errors or missing/extra vars
            self._logger.error(
                f"Error getting or formatting prompt for Grok generation: {ve}",
                exc_info=True,
            )
            return None
        except Exception as e:
            self._logger.error(
                f"LLM call failed during Grok pattern generation: {e}", exc_info=True
            )
            return None

    def _run_grok_parser(
        self, state: SimpleGrokLogParserState
    ) -> SimpleGrokLogParserState:
        ...
        if not grok_pattern:
            self._logger.error(
                f"Cannot run Grok parser for {base_name}: No Grok pattern provided in state."
            )
            return result
        ...
        try:
            ...
            self._logger.error(
                f"Invalid Grok pattern syntax provided for {base_name}: {grok_pattern} - Error: {e}",
                exc_info=True,
            )
        ...
        self._logger.info(
            f"Starting Grok parsing pass 1 (reading lines) for {base_name}..."
        )
        try:
                ...
                for i, line in enumerate(infile):
                    ...
                    if match:
                        ...
                        all_fieldnames.update(
                            match.keys()
                        )  # Dynamically collect all field names
                        parsed_count += 1
                    else:
                        if (
                            skipped_count < 10
                        ):  # Log only the first few skips to avoid flooding
                            self._logger.warning(
                                f"Line {i + 1} in {base_name} did not match Grok pattern: {line[:100]}..."
                            )
                        elif skipped_count == 10:
                            self._logger.warning(
                                f"Further Grok mismatches for {base_name} will not be logged individually."
                            )
                        skipped_count += 1
        except FileNotFoundError:
            ...
            self._logger.error(
                f"Error reading log file {base_name}: {e}", exc_info=True
            )
        ...
        self._logger.info(
            f"Grok parsing pass 1 finished for {base_name}. Parsed: {parsed_count}, Skipped: {skipped_count}"
        )

        if not parsed_data:
            self._logger.warning(
                f"No lines were successfully parsed by Grok for {base_name}. No CSV will be generated."
            )
        ...
        self._logger.info(
            f"Starting Grok parsing pass 2 (writing CSV) for {base_name} to {output_csv_path}..."
        )
        try:
            ...
            result["output_csv_path"] = (
                output_csv_path  # Set path only on successful write
            )
            self._logger.info(
                f"Successfully wrote Grok parsed data for {base_name} to {os.path.basename(output_csv_path)}"
            )
            print(
                f"--- Grok SUCCESS: {base_name} -> {os.path.basename(output_csv_path)} ---"
            )

        except IOError as e:
            self._logger.error(
                f"Failed to write CSV file {output_csv_path}: {e}", exc_info=True
            )
            print(f"--- Grok FAILED (Write Error): {base_name} ---")
            result["output_csv_path"] = ""  # Clear path on write error
        except Exception as e:
            self._logger.error(
                f"Unexpected error writing CSV for {base_name}: {e}", exc_info=True
            )
...
class SimpleDrainLogParserAgent:
    ...
    def run(
        self, initial_state: SimpleDrainLogParserState, show_progress: bool = False
        ...
        if not log_format:
            self._logger.info(
                f"Generating format for {os.path.basename(log_file_path)}..."
            )
        ...
        drain_result_state = self._run_drain_parser(
            state_for_drain, show_progress=show_progress
        )
        ...
        try:
            with open(log_file_path, "r", encoding="utf-8", errors="ignore") as f:
                ...
                if not lines or sample_size <= 0:
                    self._logger.warning(f"Cannot sample: {log_file_path}")
                    return None
                sample_logs = pd.Series(lines).sample(sample_size).tolist()
        except Exception as e:
            self._logger.error(f"Sample failed: {e}", exc_info=True)
            return None
        if not sample_logs:
            self._logger.error(f"No samples: {log_file_path}")
            return None
        try:
            prompt = self.prompts_manager.get_prompt(sample_logs=str(sample_logs))
            if not prompt:
                self._logger.error("Prompt failed.")
                return None

            class LogFormatSchema(BaseModel):
                log_format: str = Field(
                    description="Output only the log format string."
                )

            response = self._model.generate(prompt, LogFormatSchema)
            if (
                response
                and isinstance(response, LogFormatSchema)
                and response.log_format
            ):
                fmt = response.log_format.strip()
                if "<" in fmt and ">" in fmt:
                    self._logger.info(f"Generated format: {fmt}")
                    return fmt
            self._logger.warning(f"Invalid LLM format response: {response}")
            return None
        except Exception as e:
            self._logger.error(f"LLM failed: {e}", exc_info=True)
            return None

    # --- CORRECTED _run_drain_parser ---
    def _run_drain_parser(
        self, state: SimpleDrainLogParserState, show_progress: bool = False
    ) -> SimpleDrainLogParserState:
        ...
        try:
            ...
            stdout_manager = (
                nullcontext()
                if show_progress
                else redirect_stdout(open(os.devnull, "w"))
            )
            # --------------------------------------

            # Print start marker only if suppressing (otherwise Drain prints its own)
            if not show_progress:
                print(f"--- Parsing: {base_name} ---", flush=True)
            ...
            with stdout_manager:  # Apply the chosen context manager
                if file_exists:
                    ...
                    self._logger.error(
                        f"Input file not found for Drain: {file_to_parse}"
                    )
            ...
            if os.path.exists(temp_output):
                ...
                try:
                    ...
                    self._logger.info(
                        f"Successfully parsed {base_name} -> {os.path.basename(final_output_path)}"
                    )
                except OSError as rename_error:
                    self._logger.error(f"Failed rename: {rename_error}")
                    result["output_csv_path"] = (
                        ""  # Failed rename means no valid output
                    )
            else:
            ...
            self._logger.error(
                f"Drain parsing failed for {base_name}: {e}", exc_info=True
            )
            print(
                f"--- Drain FAILED for: {base_name} (Error: {e}) ---"
            )  # Always show failure
            result["output_csv_path"] = ""
...
class GroupLogParserAgent:
    ...
    def fetch_groups(self) -> Optional[Dict[str, List[str]]]:
        ...
        try:
            ...
            if not groups_data:
                self._logger.warning(
                    f"No group info found in index '{groups_index}'. Run 'collect' first?"
                )
                return None
            ...
            for doc in groups_data:
                ...
                if group_name and isinstance(files, list):
                    ...
                    self._logger.warning(
                        f"Skipping invalid group document in index '{groups_index}': ID {doc.get('_id')}"
                    )
            ...
            self._logger.error(
                f"Failed to fetch or process groups from index '{cfg.INDEX_GROUP_INFOS}': {e}",
                exc_info=True,
            )
            return None

    def parse_all_logs(
        self, groups: Dict[str, List[str]], num_threads: int, show_progress: bool
    ) -> Dict[str, List[str]]:
        ...
        group_grok_patterns: Dict[
            str, Optional[str]
        ] = {}  # Store pre-determined patterns
        total_files = 0
        pattern_agent: Optional[SimpleGrokLogParserAgent] = (
            None  # Agent instance for pattern generation
        )
        ...
        if should_predetermine:
            try:
                ...
                self._logger.error(
                    f"Failed to initialize SimpleGrokLogParserAgent for pattern pre-determination: {model_init_err}"
                )
                pattern_agent = None
        ...
        for group, files in groups.items():
            ...
            first_file_in_group = next(
                (f for f in files if isinstance(f, str) and os.path.isfile(f)), None
            )

            if first_file_in_group and pattern_agent:
                self._logger.debug(
                    f"Attempting pattern pre-determination for group '{group}' using file '{os.path.basename(first_file_in_group)}'"
                )
                try:
                    # *** Call the Grok pattern generation method ***
                    group_pattern = pattern_agent._generate_grok_pattern(
                        first_file_in_group
                    )
                    if group_pattern:
                        self._logger.info(
                            f"Pre-determined Grok pattern for group '{group}': OK"
                        )
                        # print(f"  Pattern for '{group}': {group_pattern}") # Optional: print pattern
                    else:
                        self._logger.warning(
                            f"Could not pre-determine Grok pattern for group '{group}'. Will attempt per-file generation if needed."
                        )
                except Exception as format_e:
                    self._logger.error(
                        f"Error pre-determining Grok pattern for group '{group}': {format_e}",
                        exc_info=True,
                    )
            ...
            if missing_count > 0:
                self._logger.warning(
                    f"Group '{group}': Found {missing_count} missing or invalid file paths."
                )
            total_files += len(valid_files)

        if total_files == 0:
            self._logger.warning(
                "No existing log files found across all provided groups."
            )
            return {}

        self._logger.info(
            f"Prepared {total_files} files for Grok parsing using {num_threads} worker(s)."
        )
        print(
        ...
        }  # Stores successful output CSV paths
        ...
        if num_threads <= 1:
            ...
            use_progress_bar = (
                not show_progress
            )  # Use bar only if hiding detailed output
            ...
            for group, file_path in tasks:
                ...
                if use_progress_bar:
                    self._update_progress_bar(
                        processed_files_count, total_files, current_file=base_name
                    )
                else:
                    print(
                ...
                try:
                    ...
                    initial_state: SimpleGrokLogParserState = {
                        "log_file_path": file_path,
                        "grok_pattern": group_grok_patterns.get(
                            group
                        ),  # Use pre-determined pattern if available
                        "output_csv_path": "",
                        "sample_logs": "",
                        "parsed_lines": 0,
                        "skipped_lines": 0,
                    ...
                    result = sequential_agent.run(
                        initial_state, show_progress=show_progress
                    )
                    ...
                    if output_csv:
                        ...
                        if not use_progress_bar:
                            print(
                                f"[File {processed_files_count}/{total_files}] SUCCESS: {base_name} -> {os.path.basename(output_csv)}"
                            )
                    elif not use_progress_bar:
                        print(
                            f"[File {processed_files_count}/{total_files}] FAILED: {base_name} (Parsed: {result.get('parsed_lines', 0)}, Skipped: {result.get('skipped_lines', 0)})"
                        )
                except Exception as e:
                    self._logger.error(
                        f"Sequential Grok run failed for {base_name}: {e}",
                        exc_info=True,
                    )
                    if not use_progress_bar:
                        print(
                            f"[File {processed_files_count}/{total_files}] ERROR (Agent): {base_name} - {e}"
                        )

            if use_progress_bar:
                self._update_progress_bar(
                    total_files, total_files, force_newline=True
                )  # Final newline for the bar

        else:
            # --- Parallel Execution ---
            self._logger.info(
                f"Running Grok parsing in parallel mode (max_workers={num_threads})."
            )
            ...
            with concurrent.futures.ProcessPoolExecutor(
                ...
                for future in concurrent.futures.as_completed(future_to_task):
                    ...
                    try:
                        ...
                        if output_path:
                            print(
                                f"[File {processed_files_count}/{total_files}] SUCCESS: {base_name} (Group: {original_group}) -> {os.path.basename(output_path)}",
                                flush=True,
                            )
                            ...
                            if original_group in progress:
                                ...
                                self._logger.warning(
                                    f"Group '{original_group}' not found in progress dict while processing parallel result."
                                )
                        else:
                            print(
                                f"[File {processed_files_count}/{total_files}] FAILED: {base_name} (Group: {original_group})",
                                flush=True,
                            )
                    except Exception as e:
                        # Log the error associated with the future/worker
                        self._logger.error(
                            f"Error processing Grok future for {base_name}: {e}",
                            exc_info=True,
                        )
                        print(
                            f"[File {processed_files_count}/{total_files}] ERROR (Future): {base_name} - {e}",
                            flush=True,
                        )
    ...
    def _update_progress_bar(
        ...
        progress_line += " " * (clear_len - len(progress_line))
    ...
    def run(self, num_threads: int = 1, show_progress: bool = False) -> dict:
        ...
        return self.parse_all_logs(
            groups, num_threads=effective_num_threads, show_progress=show_progress
        )
