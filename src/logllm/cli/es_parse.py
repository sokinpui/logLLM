...
def handle_es_parse_list(args):
    ...
    logger.info(f"Executing es-parse list: group={args.group}, all_history={args.all}, group_name_only={args.group_name}, json_output={args.json}")
    ...
    if group_name_only and json_output:
         logger.warning("Both --group-name and --json requested. Outputting group names as JSON list.")
    ...
    try:
        ...
        if group_name_only:
            ...
            group_field_keyword = "group_name.keyword" # Use keyword field for aggregation
            try:
                ...
                response = db.instance.search(
                    ...
                    aggs={
                        "unique_groups": {
                            "terms": {"field": group_field_keyword, "size": 10000} # Adjust size if needed
                        }
                    }
                )
                group_names = [bucket["key"] for bucket in response.get("aggregations", {}).get("unique_groups", {}).get("buckets", [])]
                ...
                logger.error(f"Error fetching unique group names: {agg_e}", exc_info=True)
        ...
        if group_filter:
            logger.info(f"Fetching history for group: {group_filter} using field '{group_field_keyword}'")
            search_params["body"] = {
            ...
            logger.info(f"Fetching all history for all groups using field '{group_field_keyword}' for sorting.")
            print("Fetching all history entries for all groups...")
            search_params["body"] = {
                "query": {"match_all": {}},
                "sort": [
                    {group_field_keyword: "asc"},
                    {"timestamp": "desc"}
                ]
            }
            ...
            logger.info(f"Fetching the latest result for each group using aggregation on field '{group_field_keyword}'.")
            ...
            results_hits = sorted(temp_results, key=lambda x: x.get('_source', {}).get('group_name', ''))
...
def handle_es_parse_use(args):
    ...
    logger.info(f"Executing es-parse use: group='{target_group}', time='{target_time_str}'")
    print(f"Attempting to re-run parsing for group '{target_group}' using pattern from run recorded around '{target_time_str}'...")
    ...
    try:
        ...
        if not db.instance.indices.exists(index=history_index):
            logger.error(f"History index '{history_index}' not found for 'use' command.")
            print(f"Error: Grok results history index '{history_index}' does not exist. Cannot find pattern.")
            return
        ...
        for hit in hits:
            ...
            try:
                ...
                if formatted_time == target_time_str:
                    ...
                    logger.info(f"Found matching history entry at {timestamp_iso} for group '{target_group}'.")
                    break # Stop searching once found
            except ValueError:
                logger.warning(f"Could not parse timestamp {timestamp_iso} from history entry {hit.get('_id')}")
                continue # Skip entry if timestamp is invalid

        if not found_entry:
            logger.error(f"Could not find history entry for group '{target_group}' matching time '{target_time_str}'.")
            print(f"Error: Could not find a history entry for group '{target_group}' recorded at '{target_time_str}'. Check 'list' output.")
            return

        if not found_pattern or found_pattern == "N/A":
            logger.error(f"History entry found, but no valid 'grok_pattern_used' field: {found_pattern}")
            print(f"Error: History entry found, but it does not contain a valid Grok pattern to use.")
            return
        ...
        single_group_config: Dict[str, Any] = {
            "group_name": target_group,
            "field_to_parse": args.field or 'content', # Use default or CLI arg if provided
        ...
        pattern_used = final_group_state.get("current_grok_pattern", "N/A") # Should be the historical one
        # Fallback status shouldn't occur here normally, but handle just in case
        if status == "success_fallback": pattern_used = "Fallback: " + SingleGroupParserAgent.FALLBACK_PATTERN
        elif "failed" in status: pattern_used = f"Failed (Pattern Used: {found_pattern})"
        ...
        logger.info(f"'es-parse use' finished for group '{target_group}'. Final Status: {status}")


    except Exception as e:
        logger.error(f"An critical error occurred during 'es-parse use': {e}", exc_info=True)
        print(f"\nAn critical error occurred: {e}")
        import traceback
        traceback.print_exc()
