...
try:
    ...
    from ..agents.parser_agent import GroupLogParserAgent, SimpleGrokLogParserAgent, SimpleGrokLogParserState
...
def handle_parse(args):
    ...
    if args.directory:
        ...
        if args.grok_pattern: # Check the renamed argument
            logger.warning("--grok-pattern is ignored when using -d/--directory parsing.")
            print("Warning: --grok-pattern option is only applicable with -f/--file.")

        logger.info(f"Executing group Grok parse command for directory '{args.directory}'. Threads: {num_threads}")
        print(f"Starting Grok parsing for all logs associated with collected groups using {num_threads} worker(s)...")
        ...
        try:
            ...
            parsing_results = agent.run(num_threads=num_threads, show_progress=show_agent_progress)
            ...
            for group, csv_paths in parsing_results.items():
                if csv_paths and isinstance(csv_paths, list) and len(csv_paths) > 0 :
                    print(f"Group '{group}': {len(csv_paths)} CSVs generated successfully")
                    successful_groups += 1
                    total_csvs += len(csv_paths)
                else:
                    # Handle cases where parsing might have failed for all files in the group
                    print(f"Group '{group}': No CSVs generated or parsing failed (check logs)")
            ...
            logger.error(f"An error occurred during group Grok parsing: {e}", exc_info=True)
        ...
        prep_message = f"--- Preparing to Grok parse single file '{os.path.basename(file_path)}'"
        if grok_pattern:
        ...
        try:
            ...
            if output_csv:
                ...
                if not args.grok_pattern and final_pattern != "N/A": # Check if pattern was generated
                ...
                if final_pattern == "N/A":
                     ...
                     print("Reason: The Grok pattern did not match any lines in the file.")
                     print(f"Pattern used: {final_pattern}")
                elif parsed > 0:
                     print("Reason: Parsing started but may have encountered an error during processing or writing CSV.")
                     print(f"Pattern used: {final_pattern}")
            ...
            logger.error(f"An error occurred during single file Grok parsing: {e}", exc_info=True)
