...
def handle_pm_scan(args):
    ...
    logger.info(f"Executing pm scan: dir={args.directory}, recursive={args.recursive}, hard={args.hard}")
    if args.hard:
        if args.recursive:
...
def handle_pm_add(args):
    ...
    success = prompts_manager.add_prompt(args.key, prompt_value, commit_message=args.message)
...
def handle_pm_version(args):
    ...
    prompts_manager = _get_prompts_manager(args)
    logger.info(f"Executing pm version: key={args.key}, verbose={args.verbose_hist}, tail={args.tail}, free={args.free}")
...
def handle_pm_revert(args):
    ...
    prompts_manager = _get_prompts_manager(args)
    logger.info(f"Executing pm revert: commit={args.commit}, key={args.key}, verbose={args.verbose_rev}")
...
def handle_pm_diff(args):
    ...
    prompts_manager = _get_prompts_manager(args)
    logger.info(f"Executing pm diff: c1={args.commit1}, c2={args.commit2}, key={args.key}, verbose={args.verbose_diff}")
...
def register_pm_parser(subparsers):
    ...
    pm_parser.add_argument(
        "--verbose-pm", action="store_true",
        help="Print the entire prompt store content after pm actions (scan, list, add, delete, revert)."
    )


    pm_subparsers = pm_parser.add_subparsers(dest="pm_action", help="Prompts Manager action", required=True)

    # --- Replicate argparse setup from original prompts_manager.py main() ---

    # 'scan' action
    scan_parser = pm_subparsers.add_parser("scan", help="Scan a directory to update the prompt store")
    ...
    scan_parser.add_argument("--hard", action="store_true", help="Perform a hard update (removes keys not found)")
    ...
    # 'add' action
    add_parser = pm_subparsers.add_parser("add", help="Add or update a prompt for an existing key")
    add_parser.add_argument("-k", "--key", type=str, required=True, help="The key to update (e.g., 'module.class.func')")
    ...
    # 'version' action
    version_parser = pm_subparsers.add_parser("version", help="List version history of prompts")
    version_parser.add_argument("-k", "--key", type=str, help="Key to show version history for")
    version_parser.add_argument("--verbose-hist", type=int, nargs="?", const=50, default=50, # Renamed
    ...
    # 'revert' action
    revert_parser = pm_subparsers.add_parser("revert", help="Revert to a previous version")
    ...
    revert_parser.add_argument("--verbose-rev", type=int, nargs="?", const=50, default=50, # Renamed
    ...
    # 'diff' action
    diff_parser = pm_subparsers.add_parser("diff", help="Show diff between two commits for the prompt store")
    ...
    diff_parser.add_argument("--verbose-diff", type=int, nargs="?", const=50, default=50, # Renamed
