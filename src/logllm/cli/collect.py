...
def handle_collect(args):
    ...
    try:
        ...
        logger.info(f"Found {len(collector.collected_files)} log files. Starting insertion...")
...
def register_collect_parser(subparsers):
    collect_parser = subparsers.add_parser('collect', help='Collect logs from a directory and insert into Elasticsearch')
    collect_parser.add_argument('-d', '--directory', type=str, required=True, help='Path to the directory containing log files')
