...
def pydantic_to_google_tool(pydantic_model: Type[BaseModel]) -> Tool:
    ...
    for name, prop_schema in properties.items():
        ...
        if google_type != GoogleApiType.TYPE_UNSPECIFIED:
            ...
            print(
                f"Warning: Could not map Pydantic type for property '{name}'. Schema details: {prop_schema}"
            )
...
class LLMModel:
    ...
    def _wait_for_rate_limit(self):
        ...
        if wait_needed > 0:
            self._logger.debug(
                f"Rate limit check: Waiting for {wait_needed:.2f} seconds."
            )
...
class GeminiModel(LLMModel):
    ...
    def __init__(self, model_name: str | None = None):
        ...
        self.rpm_limit = MODEL_RPM_LIMITS.get(
            api_model_name_key, MODEL_RPM_LIMITS["default"]
        )

        if self.rpm_limit <= 0:
            self._logger.warning(
                f"RPM limit for {self.model_name} is zero or invalid. Disabling rate limiting wait."
            )
            self.min_request_interval = 0
        else:
            self.min_request_interval = 60.0 / self.rpm_limit
        self._logger.info(
            f"Initialized Direct API GeminiModel: {self.model_name}. Rate limit: {self.rpm_limit} RPM (Min interval: {self.min_request_interval:.2f}s)"
        )
        ...
        try:
            ...
            os.environ["GOOGLE_API_KEY"] = (
                api_key  # For Langchain Embeddings if it checks
            )
            self.embedding = GoogleGenerativeAIEmbeddings(
                model="models/text-embedding-004", task_type="retrieval_document"
            )
            self._logger.info(
                f"Gemini model {self.model_name} initialized successfully via google-generativeai."
            )
    ...
    def token_count(self, prompt: str | None) -> int:
        ...
        try:
            ...
            self._logger.warning(
                f"Could not count tokens using google-generativeai for model {self.model_name}: {e}. Falling back to basic estimate."
            )
    ...
    def generate(self, prompt: str, schema: Optional[Type[BaseModel]] = None):
        ...
        if schema:
            try:
                ...
                }  # Use the nested structure for mode
                self._logger.debug(
                    f"Attempting structured output with schema tool: {schema.__name__}"
                )
            except Exception as e:
                self._logger.error(
                    f"Failed to convert Pydantic schema {schema.__name__} to Google Tool: {e}",
                    exc_info=True,
                )  # Log traceback
                tools = None
                tool_config = None
        ...
        try:
            ...
            if schema and response.candidates and response.candidates[0].content.parts:
                ...
                if function_call_part:
                    ...
                    if fc.name != schema.__name__:
                        self._logger.warning(
                            f"Model called function '{fc.name}' but expected '{schema.__name__}'. Attempting to parse anyway."
                        )
                    ...
                    try:
                        ...
                        self._logger.debug(
                            f"Extracted args from function call: {args_dict}"
                        )
                    except Exception as dict_conv_error:
                        self._logger.error(
                            f"Failed to convert function call args to dict: {dict_conv_error}",
                            exc_info=True,
                        )
                        self._logger.error(
                            f"Raw args object type: {type(fc.args)}, content: {fc.args}"
                        )
                    ...
                    try:
                        ...
                        self._logger.debug(
                            f"Successfully parsed response into schema: {schema.__name__}"
                        )
                        return parsed_schema_obj
                    except Exception as pydantic_error:
                        self._logger.error(
                            f"Pydantic validation failed for schema {schema.__name__}: {pydantic_error}",
                            exc_info=True,
                        )
                        self._logger.error(
                            f"Dict args that failed validation: {args_dict}"
                        )
                        return None
                else:
                    self._logger.warning(
                        f"Schema {schema.__name__} provided, but model did not return a function call. Checking for text fallback."
                    )
            ...
            try:
                ...
                self._logger.warning(
                    "Response blocked due to safety settings or other reasons (ValueError accessing .text)."
                )
                try:
                ...
                self._logger.error(
                    "Unexpected response structure, cannot access .text", exc_info=True
                )
                return None
            except Exception as text_extract_err:
                self._logger.error(
                    f"Unexpected error extracting text from response: {text_extract_err}",
                    exc_info=True,
                )
                return None
            ...
            self._logger.error(
                f"Error during Gemini API call (google-generativeai): {e}",
                exc_info=True,
...
def main():
    ...
    class TestSchemaDirect(BaseModel):
        ...
        numbers_list: List[int] = Field(
            ..., description="A list containing exactly 5 different integer numbers."
        )
        comment: Optional[str] = Field(
            None, description="An optional comment about the list."
        )
    ...
    try:
        ...
        if isinstance(structured_response, TestSchemaDirect):
            ...
            print(
                f"Structured response was not the expected Pydantic object, it was text: {structured_response}"
            )
